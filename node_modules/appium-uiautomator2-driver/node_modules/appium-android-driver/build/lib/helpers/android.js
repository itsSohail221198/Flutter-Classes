...
exports.prepareAvdArgs = exports.ensureNetworkSpeed = exports.SETTINGS_HELPER_ID = exports.APP_STATE = exports.helpers = void 0;
...
Object.defineProperty(exports, "SETTINGS_HELPER_ID", { enumerable: true, get: function () { return io_appium_settings_1.SETTINGS_HELPER_ID; } });
...
const AndroidHelpers = {
    ...
    async ensureDeviceLocale(adb, language, country, script) {
        const settingsApp = new io_appium_settings_1.SettingsApp({ adb });
    ...
    async initUnicodeKeyboard(adb) {
        ...
        logger_1.default.debug(`Setting IME to '${io_appium_settings_1.UNICODE_IME}'`);
        await adb.enableIME(io_appium_settings_1.UNICODE_IME);
        await adb.setIME(io_appium_settings_1.UNICODE_IME);
        return defaultIME;
    },
    async hideKeyboard(adb) {
        logger_1.default.debug(`Hiding the on-screen keyboard by setting IME to '${io_appium_settings_1.EMPTY_IME}'`);
        await adb.enableIME(io_appium_settings_1.EMPTY_IME);
        await adb.setIME(io_appium_settings_1.EMPTY_IME);
    },
    ...
    async pushSettingsApp(adb, throwError, opts) {
        ...
        try {
            await AndroidHelpers.installHelperApp(adb, io_appium_settings_1.path, io_appium_settings_1.SETTINGS_HELPER_ID);
        }
        catch (err) {
            if (throwError) {
                throw err;
        ...
        if (await settingsApp.isRunningInForeground()) {
            logger_1.default.debug(`${io_appium_settings_1.SETTINGS_HELPER_ID} is already running. ` +
        ...
        const fixSettingsAppPermissionsForLegacyApis = async () => {
            ...
            logger_1.default.info(`Granting permissions ${perms} to '${io_appium_settings_1.SETTINGS_HELPER_ID}'`);
            await adb.grantPermissions(io_appium_settings_1.SETTINGS_HELPER_ID, perms.map((x) => `android.permission.${x}`));
        };
        try {
    ...
    async initDevice(adb, opts) {
        ...
        if (!AndroidHelpers.isEmulator(adb, opts)) {
            if (mockLocationApp || lodash_1.default.isUndefined(mockLocationApp)) {
                await AndroidHelpers.setMockLocationApp(adb, mockLocationApp || io_appium_settings_1.SETTINGS_HELPER_ID);
            }
            else {
